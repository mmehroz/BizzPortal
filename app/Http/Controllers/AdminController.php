		...
		if($request->optradio == "awaiting"){
			...
			try{
				...
				function ($message) use ($emailcandi) {
					...
					$message->subject('Invitation to Interview - Bizz World Communications');
				});
				$updated = DB::connection('mysql')->table('jobapplicant')
				->where('jobapplicant_id','=',$request->editempid)
				->update([
				...
				// try{
					...
					try{
						...
						function ($message) use ($candiname,$task) {
							...
							$message->cc('recruitment@bizzworldcommunications.com');
							$message->subject('Offer Letter for '.$task->evu_cantechdesgdept.' Bizz World Communications');
						});
						$update = DB::connection('mysql')->table('jobapplicant')
		...
		// dd($request);
		   ...
		   'jobapplicant_ChangeBy' => session()->get("name"),
			...
			if($request->can_finalstatus == "evaluateByManager"){
				...
				//  $message->cc('recruitment@bizzworldcommunications.com');
				//  $message->subject('Appointment Letter for '.$task->jobapplicant_postionapppliedform.' Bizz World Communications');
				// });
		   ...
		   'jobapplicant_ChangeBy' => session()->get("name"),
			...
			if($action[0] == "attend"){
				try{
					...
					$message->cc('recruitment@bizzworldcommunications.com');
					$message->subject('Thank You for Interviewing with Bizz World Communications');
					});
				}catch ( \Exception $e ) {
					echo json_encode(true);
				}
			...
			// 		$message->to($candidateemail);
			// 		$message->cc('recruitment@bizzworldcommunications.com');
			// 		$message->subject('Invitation to Reschedule Interview - Bizz World Communications');
			// 		});
			// 	}catch ( \Exception $e ) {
			// 		echo json_encode(true);
			// 	}
